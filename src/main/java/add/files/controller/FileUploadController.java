...
import add.files.util.FileValidator;
import add.files.util.MultiFileValidator;
...
public class FileUploadController {
	...
	private static String UPLOAD_LOCATION2 = "H:/studia info/java/workspace_STS/glider2/src/main/webapp/resources/addedImages/";
	...
	public String singleFileUpload(@Valid FileBucket fileBucket, BindingResult result, ModelMap model) throws IOException {
		...
		if (result.hasErrors()) {
			...
			FileCopyUtils.copy(fileBucket.getFile().getBytes(),
					new File(UPLOAD_LOCATION2 + fileBucket.getFile().getOriginalFilename()));
